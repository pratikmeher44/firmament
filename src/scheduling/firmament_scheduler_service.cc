...
class FirmamentSchedulerServiceImpl final :
  ...
  Status NodeAdded(ServerContext* context,
    ...
    root_rtnd_ptr = root_rs_ptr->mutable_topology_node();
    ...
    cpu_stats.set_cpu_allocatable(rtnd_ptr->resource_desc().resource_capacity().cpu_cores());
    //resource_stats.cpus_stats(0).set_cpu_utilization(1.0);
    ...
    resource_stats.set_mem_capacity(rtnd_ptr->resource_desc().resource_capacity().ram_cap());
    //resource_stats.set_mem_utilization(1.0);
